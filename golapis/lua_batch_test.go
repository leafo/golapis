...
func benchCaptureResponseData() CaptureResponse {
...
func benchQueryArgsData() []queryArg {
	return []queryArg{
		{key: "page", value: "1"},
...
func BenchmarkCaptureResponse_Individual(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		pushCaptureResponseIndividual(L, cr)
		luaPop(L, 1)
	}
}
...
func BenchmarkQueryArgs_Batch(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		pushQueryArgsToLuaTable(L, args)
		luaPop(L, 1)
	}
}
...
func BenchmarkQueryArgs_Individual(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		pushQueryArgsIndividual(L, args)
		luaPop(L, 1)
	}
}
