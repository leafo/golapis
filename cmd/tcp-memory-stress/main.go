...
func main() {
	iterations := flag.Int("iterations", 10000, "number of TCP request iterations")
	...
	fmt.Println("TCP Memory Stress Test")
	fmt.Println("======================")
	...
	for i := 0; i < count; i++ {
		code := fmt.Sprintf(`
			golapis.timer.at(0, function()
				...
				local ok, err = sock:connect("127.0.0.1", %d)
				if ok then
					sock:send("ping")
					sock:receive(4)
				end
	...
	go func() {
		for {
			...
			case <-server.done:
				return
			default:
				// Set accept deadline so we can check done channel
...
func handleConnection(conn net.Conn) {
	defer conn.Close()
	io.Copy(conn, conn) // Echo all data back
}
